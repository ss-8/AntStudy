...
@Controller
@RequestMapping("ssh")
public class SSHLoginController {

	@RequestMapping("login/{dest}/")
	public String sshLogin(@PathVariable String dest,Model model){
		try {
			Thread.sleep(3000);
		...
		String sshCommand="ssh root@172.18.126.238";
		String password = "nFslTb67oRHsioidIZXwqGk!NO8W6FtW#*Tz*dwrhFNRVDXtqBRt&KZWh*8lC8Zz";
		if(processKey(sshCommand)){
			try {
				Thread.sleep(2000);
				processKey(password);
		...
		model.addAttribute("message", "完事了");
		return dest;
	}
